...
def create_modern_sns_image(race_info, df_sorted):
    ...
    draw.text((40, 30), f"🎯 {race_info['place']} {race_info['num']}R 事前予想", font=f_title, fill=(255, 255, 255))
        ...
        if "base_df" in st.session_state:
            ...
            if st.button("📈 過去データから最適重みを抽出", use_container_width=True):
                with st.spinner("統計解析中..."):
                    ...
                    st.write(f"🔍 有効データ: **{len(clean_df)}**件")
                    if len(clean_df) >= 10:
            ...
            if "auto_weights" in st.session_state:
                fig = px.pie(names=list(st.session_state["auto_weights"].keys()), 
                             values=list(st.session_state["auto_weights"].values()), hole=.4)
...
with tab_sns:
    if "analytica_result" in st.session_state:
        if st.button("✨ モダンデザイン画像を生成", use_container_width=True):
            ...
            st.download_button("💾 画像を保存", buf.getvalue(), f"yoso_{r_num}.png", "image/png")
    else:
        st.info("先に予想を確定させてください")
